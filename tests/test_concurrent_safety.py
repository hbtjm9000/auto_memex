...
from .conftest import INGEST_SCRIPT, QUEUE, QUEUE_MANAGER
...
def test_fcntl_lock_present():
    ...
    # Must use flock
    assert "flock(" in content or "fcntl.flock" in content, "Should call flock() function"

    # Should use exclusive lock for writes
    assert "LOCK_EX" in content, "Should use LOCK_EX for exclusive locking during writes"
...
def test_two_workers_no_corruption_serial():
    ...
    # First worker pop
    result1 = subprocess.run(["python3", str(QUEUE_MANAGER), "pop"], capture_output=True, text=True)
    ...
    if "worker-serial" in output1:
        ...
        # Mark done
        subprocess.run(
            ["python3", str(QUEUE_MANAGER), "complete", task_id], capture_output=True, text=True
        )

    # Second worker pop
    result2 = subprocess.run(["python3", str(QUEUE_MANAGER), "pop"], capture_output=True, text=True)
    ...
    assert statuses.get("worker-serial-a") == "Done", f"Task A should be Done, got: {statuses}"
    assert statuses.get("worker-serial-b") == "Done", f"Task B should be Done, got: {statuses}"
...
def test_queue_lock_blocks_other_writers():
    ...
    assert "LOCK_UN" in content, "Should release lock with fcntl.LOCK_UN"
    ...
    for i in range(5):
        result = subprocess.run(
            [
                ...
                str(INGEST_SCRIPT),
                "--url",
                f"https://example.com/concurrent-test-{i}",
                "--type",
                "concept",
            ],
            capture_output=True,
...
def test_lock_timeout_prevents_deadlock():
    ...
    assert "LOCK_EX" in content, "Should use exclusive lock (LOCK_EX) for write operations"
...
def test_queue_integrity_after_interrupted_write():
    ...
    try:
        ...
        result = subprocess.run(
            ["python3", str(QUEUE_MANAGER), "stats"], capture_output=True, text=True
        )
...
def test_concurrent_queue_operations_structural_safety():
    ...
    for _ in range(10):
        result = subprocess.run(
            ["python3", str(QUEUE_MANAGER), "pop"], capture_output=True, text=True
        )
