...
from .conftest import INDEX, INGEST_SCRIPT, LINT_SCRIPT, LOG, QUEUE_MANAGER, VAULT
...
"""
    ...
    assert len(matching) == 1, f"Should have exactly 1 file, found {len(matching)}: {matching}"
...
def test_index_no_duplicate_entry(temp_page, sample_frontmatter, clean_index):
    ...
    temp_page("concepts", filename, sample_frontmatter + "\n# Idempotent Index\nContent.\n")
    ...
    try:
        ...
        if INDEX.exists():
            ...
            assert count <= 1, f"Duplicate index entries found: {content}"
...
def test_worker_idempotent_on_double_run(temp_page, sample_frontmatter, clean_queue):
    ...
    # First pop
    subprocess.run(["python3", str(QUEUE_MANAGER), "pop"], capture_output=True, text=True)
    ...
    if "idempotency-test" in output:
        ...
        raise AssertionError("Same task was returned twice - not idempotent")
...
def test_multiple_concurrent_ingests_dont_corrupt_queue():
    ...
    urls = [f"https://example.com/idempotent-seq-{i}" for i in range(3)]
...
def test_reopen_already_open_task_not_allowed():
    ...
    result = subprocess.run(["python3", str(QUEUE_MANAGER), "pop"], capture_output=True, text=True)
