...
def test_enqueue_adds_task(clean_queue):
    ...
    result = subprocess.run(
        [
            ...
            str(INGEST_SCRIPT),
            "--url",
            "https://example.com/test-enqueue-123",
            "--type",
            "concept",
        ],
        capture_output=True,
...
def test_worker_marks_done(clean_queue):
    ...
    assert task["status"] == "Open", f"Popped task should be Open, got: {task['status']}"

    # Mark as done
    result = subprocess.run(
        ["python3", str(QUEUE_MANAGER), "complete", "deadbeef"], capture_output=True, text=True
    )
...
def test_queue_lock_prevents_corruption():
    ...
    assert "LOCK_EX" in content or "LOCK_SH" in content, "Should use exclusive or shared lock"
