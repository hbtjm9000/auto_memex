...
"""
    ...
    temp_page("concepts", "random-tag-test", content)
...
"""
    ...
    temp_page("concepts", "valid-tag-test", content)
    ...
    lines = [
        ...
        if "valid-tag-test" in line and "zero-trust" in line.lower() and "warning" in line.lower()
    ]
    assert len(lines) == 0, f"Valid tag should not warn: {lines}"
...
"""
    ...
    temp_page("concepts", "multi-unlisted-test", content)
    ...
    assert flagged_count >= 2, f"Expected at least 2 unlisted tags flagged, got: {output}"
...
def test_all_taxonomy_tags_valid(temp_page, sample_frontmatter, taxonomy_tags):
    ...
    for tag in key_tags:
        ...
        temp_page("concepts", f"tag-{tag}-test", content)

        result = run_lint_vault()
        output = result.stdout + result.stderr

        # Should not warn about this tag for THIS file
        # Exclude Rule 1 (orphan) warnings since those are expected for isolated test pages
        lines = [
            line
            for line in output.split("\n")
            if f"tag-{tag}-test" in line
            and "warning" in line.lower()
            and tag in line.lower()
            and "Rule 6" in line
        ]  # Only taxonomy warnings (Rule 6), not orphan (Rule 1)
        assert len(lines) == 0, f"Valid tag '{tag}' should not warn: {lines}"
...
"""
    ...
    temp_page("concepts", "custom-tag-test", content)

    result = run_lint_vault()
    output = result.stdout + result.stderr

    # Custom tag not in schema should be flagged
    lines = [line for line in output.split("\n") if "custom-tag-test" in line]
    has_warning = any(
        "my-custom-tag" in line.lower() or "unlisted" in line.lower() or "warning" in line.lower()
        for line in lines
    )
    assert has_warning, f"Custom tag should be flagged: {output}"
...
"""
    ...
    temp_page("concepts", "case-tag-test", content)
    ...
    lines = [
        line
        for line in output.split("\n")
        if "case-tag-test" in line and "zero-trust" in line.lower() and "warning" in line.lower()
    ]
    assert len(lines) == 0, f"Case variant of valid tag should not warn: {lines}"
