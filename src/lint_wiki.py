...
Lints the Obsidian vault for wiki hygiene issues.
Vault path configurable via WIKI_VAULT environment variable.
...
class ScanResult:
    ...
    def add_issue(self, issue: Issue):
        ...
        if issue.file_path:
            self.files_with_issues = len({i.file_path for i in self.issues if i.file_path})
...
def parse_frontmatter(content: str) -> tuple[Frontmatter | None, int]:
        ...
        if ":" in line:
            ...
            if key == "title":
                ...
                if match:
                    ...
                    fm.contradictions = [
                        c.strip().strip("\"'") for c in contrad_str.split(",") if c.strip()
                    ]
        ...
        if len(inbound) == 0:
            issues.append(
                Issue(
                    ...
                    message=f"Orphan page: no inbound wikilinks ({rel_path})",
                    file_path=f,
                    fix_available=True,
                )
            )
...
def check_index_completeness(files: list[Path], vault_path: Path) -> list[Issue]:
    """Rule 3: Check for pages on disk that are missing from index.md."""
    issues = []

    if not INDEX_PATH.exists():
        # Index doesn't exist - create a special issue to trigger index creation
        issues.append(
            Issue(
                rule=3,
                severity=Severity.CRITICAL,
    ...
    for f in files:
        ...
        # Check if page is referenced in index using wikilink pattern
        # Look for [[stem]] or [[path/stem]] pattern
        stem = rel_path.stem
        # Pattern matches [[stem]] or [[something/stem]] but not [[stem-something-else]]
        ...
        if not pattern.search(index_content):
            issues.append(
                Issue(
                    rule=3,
                    severity=Severity.WARNING,
                    message="Page not found in index.md",
                    file_path=f,
                    fix_available=True,
                )
            )
...
def check_frontmatter_validation(files: list[Path], vault_path: Path) -> list[Issue]:
    ...
    for f in files:
        ...
        if fm is None:
            issues.append(
                Issue(
                    rule=4,
                    severity=Severity.CRITICAL,
                    message="Missing frontmatter (required: title, created, updated, type, tags)",
                    file_path=f,
                    fix_available=False,
                )
            )
        ...
        missing = []
        for req_field in REQUIRED_FRONTMATTER:
            if req_field == "title" and not fm.title:
                missing.append(req_field)
            elif req_field == "created" and not fm.created:
                missing.append(req_field)
            elif req_field == "updated" and not fm.updated:
                missing.append(req_field)
            elif req_field == "type":
                if not fm.page_type:
                    missing.append(req_field)
        ...
        for severity in [Severity.CRITICAL, Severity.WARNING, Severity.INFO]:
            ...
            if issues:
                ...
                for issue in sorted(issues)[:5]:  # Limit per severity
                    rel_path = (
                        issue.file_path.relative_to(vault_path) if issue.file_path else "unknown"
                    )
...
def pluralize_type(page_type: str) -> str:
    """Convert page type to plural section name.
    ...
    Examples:
        ...
        summary -> Summaries
    """
    type_lower = page_type.lower()
    # Handle irregular plurals
    if type_lower == "entity":
        return "Entities"
    elif type_lower == "query":
        return "Queries"
    elif type_lower == "summary":
        return "Summaries"
    # Regular pluralization: add 's'
    return page_type.title() + "s"
...
"""
    ...
    # First: deduplicate existing entries
    ...
    for line in lines:
        if line.strip().startswith("- [["):
            # Extract the page reference for deduplication
            match = re.search(r"\[\[([^\]]+)\]\]", line)
            if match:
                entry_key = match.group(1).lower()
    ...
    if duplicates_removed > 0:
        index_content = "\n".join(deduped_lines)
        changes.append(f"Removed {duplicates_removed} duplicate index entries")
    ...
    for issue in result.issues:
        ...
        # Special case: index.md doesn't exist, create it
        if not issue.file_path:
            # Index creation is handled at the start of this function
            continue

        stem = issue.file_path.stem

        # Re-check if already in index using the deduped content
        # Look for exact [[stem]] pattern (not partial matches)
        exact_pattern = re.compile(r"\[\[" + re.escape(stem) + r"\]\]", re.IGNORECASE)
        if exact_pattern.search(index_content):
            continue
        ...
        section_header = f"## {pluralize_type(page_type)}"
        ...
        for i, line in enumerate(lines):
            ...
            if stripped == section_header:
                # Found section header, insert after blank line following header
                insert_idx = i + 1
